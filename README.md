...
- **Sentences course** — 12 kid-sized grammar patterns (これは〜です, 〜がすき, 〜をください…) with 60 sentences and Duolingo-style "arrange the tiles" quizzes. Unlocks after 5 hiragana lessons.
- **Games hub** — 4 replayable mini-games built from whatever she has learned so far: Word Builder (spell words from kana tiles), Kana Pop (pop the bubble making the sound — uses the real recordings), Memory Match (flip-card pairs), and Cat Race (speed quiz vs rival cats). Full fish rewards for the first 3 plays of each game per day, small treats after.
- **Cat Home** — a cozy room where all unlocked cats live together: they wander, nap, and play; day/night follows the real clock. Tap a cat and it "says" a word she's learned (with audio); spend a fish to give treats. Every cat wears its own outfit — wardrobes are per-cat.
- **Voice settings** — the 🔊 button in the top bar picks among the device's Japanese voices and offers a slow 🐢 speech mode.
